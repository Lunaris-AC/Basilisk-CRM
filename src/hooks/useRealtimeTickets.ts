        ...
        const channel = supabase
            ...
            .on(
                ...
                async (payload) => {
                    ...
                    // --- Notifications enrichies (Toast Sonner) ---
                    if (payload.eventType === 'INSERT' && ticket) {
                        ...
                        toast(
                            isCritique ? `🚨 Nouveau ticket : ${priorityLabel}` : `📩 Nouveau ticket : ${priorityLabel}`,
                            {
                        ...
                        if (ticket.assignee_id === user?.id && old.assignee_id !== user?.id) {
                            toast('📌 Ticket assigné', {
                                description: `Le ticket "${ticket.title || 'Sans titre'}" vous a été assigné.`,
                                duration: 6000,
                            })
                        }
                    }
