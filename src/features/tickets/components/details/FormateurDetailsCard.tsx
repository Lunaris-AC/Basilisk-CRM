        ...
        <div className="p-6 rounded-2xl bg-white/5 border border-emerald-500/20 backdrop-blur-md shadow-xl space-y-4 relative group overflow-hidden transition-all duration-300 hover:border-emerald-500/40">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 relative z-10 pt-2">
                <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium text-white capitalize">{formattedDate || <span className="text-white/30 italic">Non renseignée</span>}</p>
                </div>
                <div className="space-y-1">
                    <p className="text-xs text-white/50 flex items-center gap-1.5 mb-1">
                        <MapPin className="w-3.5 h-3.5 text-emerald-400/70" /> Lieu
                    </p>
                    <p className="text-sm font-medium text-white">{details?.training_location || <span className="text-white/30 italic">Non renseigné</span>}</p>
                </div>
                <div className="space-y-1">
                    <p className="text-xs text-white/50 flex items-center gap-1.5 mb-1">
                        <GraduationCap className="w-3.5 h-3.5 text-emerald-400/70" /> Type de formation
                    </p>
                    <p className="text-sm font-medium text-white">{details?.training_type || <span className="text-white/30 italic">Non renseigné</span>}</p>
                </div>
            </div>

            {/* Modale d'édition */}
            <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
                <DialogContent className="bg-zinc-900 border-emerald-500/20 text-white sm:max-w-md">
                    <DialogHeader>
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            <Label htmlFor="date" className="text-sm font-medium text-white/80">Date de déplacement</Label>
                            <Input id="date" type="date" value={date} onChange={e => setDate(e.target.value)} className="bg-black/40 border-white/10 text-white focus:ring-emerald-500/50" />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="location" className="text-sm font-medium text-white/80">Lieu de formation</Label>
                            <Input id="location" value={location} onChange={e => setLocation(e.target.value)} className="bg-black/40 border-white/10 text-white focus:ring-emerald-500/50" placeholder="Sur site, Distanciel..." />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="type" className="text-sm font-medium text-white/80">Type de formation</Label>
                            <Input id="type" value={type} onChange={e => setType(e.target.value)} className="bg-black/40 border-white/10 text-white focus:ring-emerald-500/50" placeholder="Initiale..." />
                        </div>
                    </div>
                    <DialogFooter>
