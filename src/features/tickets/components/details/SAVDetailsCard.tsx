        ...
        <div className="p-6 rounded-2xl bg-white/5 border border-rose-500/20 backdrop-blur-md shadow-xl space-y-4 relative group overflow-hidden transition-all duration-300 hover:border-rose-500/40">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 relative z-10 pt-2">
                <div className="space-y-1">
                    ...
                    <p className="text-sm font-medium text-white">{details?.serial_number || <span className="text-white/30 italic">Non renseigné</span>}</p>
                </div>
                <div className="space-y-1">
                    <p className="text-xs text-white/50 flex items-center gap-1.5 mb-1">
                        <FileText className="w-3.5 h-3.5 text-rose-400/70" /> Référence Produit
                    </p>
                    <p className="text-sm font-medium text-white">{details?.product_reference || <span className="text-white/30 italic">Non renseigné</span>}</p>
                </div>
                <div className="space-y-1 md:col-span-2 lg:col-span-1">
                    ...
                    {details?.hardware_status ? (
                        ...
                        <span className="text-sm text-white/30 italic">Non renseigné</span>
                    )}
                </div>
            </div>
            ...
            <Dialog open={isEditModalOpen} onOpenChange={setIsEditModalOpen}>
                <DialogContent className="bg-zinc-900 border-rose-500/20 text-white sm:max-w-md">
                    <DialogHeader>
                    ...
                    <div className="space-y-4 py-4">
                        <div className="space-y-2">
                            <Label htmlFor="serial" className="text-sm font-medium text-white/80">Numéro de série</Label>
                            <Input id="serial" value={serial} onChange={e => setSerial(e.target.value)} className="bg-black/40 border-white/10 text-white focus:ring-rose-500/50" placeholder="SN-..." />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="ref" className="text-sm font-medium text-white/80">Référence Produit</Label>
                            <Input id="ref" value={ref} onChange={e => setRef(e.target.value)} className="bg-black/40 border-white/10 text-white focus:ring-rose-500/50" placeholder="Modèle exact" />
                        </div>
                        <div className="space-y-2">
                            <Label htmlFor="status" className="text-sm font-medium text-white/80">État du matériel</Label>
                            <Input id="status" value={status} onChange={e => setStatus(e.target.value)} className="bg-black/40 border-white/10 text-white focus:ring-rose-500/50" placeholder="Ex: Écran cassé" />
                        </div>
                    </div>
                    <DialogFooter>
