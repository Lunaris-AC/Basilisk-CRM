    ...
    if (!documents || documents.length === 0) {
        return (
            <div className="p-12 text-center rounded-2xl bg-white/5 border border-dashed border-white/10">
                ...
                <p className="text-white/40 text-lg font-medium">Aucun document pour le moment.</p>
                {canManage && (
                    <p className="text-white/30 text-sm mt-1">Utilisez le bouton "Uploader" pour ajouter un document.</p>
                )}
            </div>
        )
    }
    ...
    return (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {documents.map((doc) => (
                <div
                    key={doc.id}
                    className="group p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md hover:bg-white/[0.08] hover:border-indigo-500/30 transition-all duration-300 flex flex-col gap-4 relative overflow-hidden"
                >
                    {/* Glow effect */}
                    <div className="absolute -right-6 -top-6 w-20 h-20 bg-indigo-500/5 rounded-full blur-2xl group-hover:bg-indigo-500/10 transition-colors" />
                    ...
                    <div className="flex items-center gap-3 relative z-10">
                        ...
                        <div className="flex-1 min-w-0">
                            <h3 className="text-sm font-bold text-white truncate group-hover:text-indigo-200 transition-colors">{doc.title}</h3>
                            <span className="text-[10px] px-2 py-0.5 rounded-md bg-white/10 text-white/50 font-semibold tracking-wider">
                    ...
                    <div className="flex items-center gap-2 pt-2 border-t border-white/5 relative z-10 mt-auto">
                        <a
                            ...
                            className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-xl bg-indigo-500/10 border border-indigo-500/20 text-indigo-300 text-xs font-bold hover:bg-indigo-500/20 hover:border-indigo-500/40 transition-all"
                        >
