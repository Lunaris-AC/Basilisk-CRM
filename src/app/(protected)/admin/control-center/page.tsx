...
export const metadata = {
    title: 'Control Center | God Mode',
    description: 'Interface d\'administration centralisée',
}
...
export default async function ControlCenterPage() {
    ...
    if ((profile?.role !== 'TECHNICIEN' || profile?.support_level !== 'N4') && profile?.role !== 'ADMIN') {
        // Renvoi brutal vers le dashboard si ce n'est pas un admin
        redirect('/dashboard')
    }
    ...
    return (
        <div className="w-full h-full max-w-7xl mx-auto p-8 animate-in fade-in duration-500">
            <ControlCenterContent users={allUsers || []} />
        </div>
    )
}
