...
const STATUS_LABELS: Record<string, { label: string; color: string }> = {
    ...
    resolu: { label: 'Résolu', color: 'bg-indigo-500/20 text-indigo-300 border-indigo-500/30' },
...
const COMPLEXITY_LABELS: Record<string, { label: string; color: string }> = {
    ...
    MAJEUR: { label: 'MAJEUR', color: 'bg-purple-500/20 text-purple-300 border-purple-500/40 shadow-[0_0_8px_rgba(168,85,247,0.2)]' },
}
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="relative">
                ...
                <div className="relative flex items-center justify-between gap-4 flex-wrap">
                    <div className="flex items-center gap-4">
                        <div className="p-3 bg-gradient-to-br from-cyan-500/20 to-violet-500/20 border border-cyan-500/30 rounded-2xl shadow-lg shadow-cyan-500/10">
                            <Code2 className="w-7 h-7 text-cyan-400" />
                        </div>
                        <div>
                            <h1 className="text-3xl font-black tracking-tight bg-gradient-to-r from-cyan-400 to-violet-400 bg-clip-text text-transparent">
                                Portail SD
                            </h1>
                            <p className="text-sm text-white/50 mt-0.5">Bugs & Évolutions — Département Développement</p>
                        </div>
                    </div>
            ...
            <div className="p-5 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md shadow-xl">
                <div className="flex items-center gap-2 mb-4 text-sm font-bold text-white/60">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                    ...
                    <div className="relative">
                        ...
                        <input
                            ...
                            className="w-full pl-10 pr-4 py-2.5 bg-black/40 border border-white/10 rounded-xl text-white text-sm placeholder-white/30 focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all"
                        />
                    </div>
                    ...
                    <select
                        ...
                        className="bg-black/40 border border-white/10 rounded-xl px-4 py-2.5 text-white text-sm focus:outline-none focus:ring-2 focus:ring-cyan-500/50 transition-all appearance-none"
                    >
                    ...
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-white/10 bg-white/5">
                                <th className="text-left px-5 py-4 text-xs font-bold text-white/50 uppercase tracking-wider">Titre</th>
                                <th className="text-center px-3 py-4 text-xs font-bold text-white/50 uppercase tracking-wider">Type</th>
                                <th className="text-center px-3 py-4 text-xs font-bold text-white/50 uppercase tracking-wider">Complexité</th>
                                <th className="text-center px-3 py-4 text-xs font-bold text-white/50 uppercase tracking-wider">Statut</th>
                                <th className="text-left px-3 py-4 text-xs font-bold text-white/50 uppercase tracking-wider">Assigné</th>
                                <th className="text-left px-3 py-4 text-xs font-bold text-white/50 uppercase tracking-wider">Date</th>
                                <th className="text-right px-5 py-4 text-xs font-bold text-white/50 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/5">
                            {sds.map(sd => {
                                const typeInfo = sd.dev_details?.type === 'BUG'
                                    ...
                                    : { icon: <Sparkles className="w-3.5 h-3.5" />, label: 'Évo', color: 'bg-violet-500/20 text-violet-300 border-violet-500/30' }
                                ...
                                return (
                                    <tr key={sd.id} className="group hover:bg-white/[0.03] transition-colors">
                                        <td className="px-5 py-4">
                                            <Link href={`/tickets/${sd.id}`} className="font-semibold text-white hover:text-cyan-300 transition-colors truncate block max-w-[300px]">
                                                {sd.title}
                                            </Link>
                                            <p className="text-xs text-white/30 mt-0.5 truncate max-w-[300px]">{sd.description}</p>
                                        </td>
                                        ...
                                        <td className="px-3 py-4">
                                            {sd.assignee ? (
                                                <span className="text-sm text-white/70">{sd.assignee.first_name} {sd.assignee.last_name}</span>
                                            ) : (
                                                <span className="text-xs text-white/20 italic">Non assigné</span>
                                            )}
                                        </td>
                                        <td className="px-3 py-4 text-xs text-white/40">
                                        ...
                                        <td className="px-5 py-4 text-right">
                                            <div className="flex items-center gap-2 justify-end">
                                                ...
                                                <Link
                                                    href={`/tickets/${sd.id}`}
                                                    className="px-3 py-1.5 rounded-lg bg-white/5 hover:bg-white/10 text-white/60 text-xs font-medium border border-white/10 transition-all flex items-center gap-1.5"
                                                >
