        ...
        return (
            <div className="flex-1 overflow-y-auto custom-scrollbar bg-gradient-to-br from-slate-900 to-black relative flex items-center justify-center h-full">
                ...
                <div className="relative z-10 text-center space-y-4">
                    ...
                    <h2 className="text-2xl font-bold text-foreground">Devis introuvable</h2>
                    <p className="text-muted-foreground">Ce devis n'existe pas ou vous n'y avez pas accès.</p>
                    <Link
                        href="/commerce"
                        className="inline-flex items-center gap-2 px-6 py-3 mt-4 bg-white/10 hover:bg-white/20 text-foreground rounded-xl transition-all"
                    >
            ...
            <div className="relative z-10 p-6 lg:p-10 flex flex-col min-h-full space-y-8 max-w-6xl mx-auto">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div className="flex items-center gap-4">
                        <Link
                            href="/commerce"
                            className="p-3 bg-white/5 hover:bg-white/10 text-foreground/70 hover:text-foreground rounded-xl transition-all"
                        >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div className="p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-xl space-y-4">
                        ...
                        <div className="space-y-2 text-foreground/80">
                            {quote.client ? (
                                <>
                                    <div className="flex items-center gap-2"><Building2 className="w-4 h-4 text-muted-foreground" /> <span className="font-medium text-foreground">{quote.client.company || 'Sans société'}</span></div>
                                    <div className="flex items-center gap-2"><User className="w-4 h-4 text-muted-foreground" /> {quote.client.first_name} {quote.client.last_name}</div>
                                </>
                            ) : (
                                <span className="text-muted-foreground italic">Client introuvable</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="p-6 rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-xl space-y-4">
                        ...
                        <div className="space-y-2 text-foreground/80">
                            {quote.store ? (
                                <>
                                    <div className="font-medium text-foreground text-lg">{quote.store.name}</div>
                                ...
                                <span className="text-muted-foreground italic">Magasin introuvable</span>
                            )}
                        </div>
                    </div>
                </div>
                ...
                <div className="rounded-2xl bg-white/5 backdrop-blur-xl border border-white/10 shadow-2xl p-6 overflow-hidden flex flex-col">
                    <h3 className="text-xl font-semibold text-foreground/90 mb-4 px-2">Détail du devis</h3>
                    <div className="rounded-xl border border-white/10 bg-black/20 overflow-hidden">
                        <Table>
                            <TableHeader className="bg-white/5 sticky top-0 z-10 backdrop-blur-md">
                                <TableRow>
                                    <TableHead className="font-semibold text-foreground/70 w-[40%]">Désignation</TableHead>
                                    <TableHead className="font-semibold text-center text-foreground/70 w-[10%]">Qté</TableHead>
                                    <TableHead className="font-semibold text-right text-foreground/70 w-[15%]">PU HT</TableHead>
                                    <TableHead className="font-semibold text-center text-foreground/70 w-[15%]">TVA</TableHead>
                                    <TableHead className="font-semibold text-right text-foreground/70 w-[20%]">Total Ligne HT</TableHead>
                                </TableRow>
                            </TableHeader>
                            <TableBody>
                                {quote.lines && quote.lines.length > 0 ? (
                                    quote.lines.map((line) => (
                                        <TableRow key={line.id} className="hover:bg-white/5 transition-colors">
                                            <TableCell className="font-medium text-foreground">{line.designation}</TableCell>
                                            <TableCell className="text-center text-foreground/80">{line.quantity}</TableCell>
                                            <TableCell className="text-right font-mono text-foreground/80">{formatCurrency(line.unit_price)}</TableCell>
                                            <TableCell className="text-center font-mono text-muted-foreground">{line.tax_rate}%</TableCell>
                                            <TableCell className="text-right font-mono font-medium text-emerald-400">{formatCurrency(line.line_total)}</TableCell>
                                        </TableRow>
                                    ))
                                ) : (
                                    <TableRow>
                                        <TableCell colSpan={5} className="py-12 text-center text-muted-foreground">Aucune ligne dans ce devis.</TableCell>
                                    </TableRow>
                                )}
                            </TableBody>
                        </Table>
                ...
                <div className="flex justify-end">
                    <div className="w-full md:w-96 space-y-4 p-8 rounded-2xl bg-black/40 backdrop-blur-xl border border-white/5 shadow-2xl">
                        <div className="flex justify-between text-foreground/70 text-lg">
                            <span>Total HT</span>
